...
`--source` pulls a **custom harness** (a git repo with a `payload/` tree plus an `omakase.manifest`) into a local cache and injects the omakase base harness's payload with the custom harness's payload layered on top — base machinery (banner, ledger, deferred-check, …) underneath, the custom harness's delta winning on overlap — so a custom harness ships only its delta and never keeps its own copy of the base harness. The custom harness is remembered: a later bare `/omakase init` refreshes and re-injects it. If the script refuses it (no `payload/`, no `omakase.manifest`, or merged hook wiring that references a `.omakase/*.sh` script neither side ships), relay the error verbatim — nothing was changed.

The manifest is flat `key: value` (`name:` required; `version:` and `recommends:` optional). If a custom harness declares `recommends:` (free text — e.g. companion plugins it pairs with), init prints it once at install (`omakase: this harness recommends — …`); relay it so the user can act on it. Init also prints how to customize: editing injected files in place is overwritten on re-init, so the supported path is forking the custom harness (clone → edit → publish) and init-ing from the copy. Surface that if the user asks how to change the harness.
